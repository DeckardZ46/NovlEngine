/**
 * < Novl Engine > gui_sink.h
 * Author: DeckardZ46
 * Date: 2024/09/24
 * Note: gui sink for Novl Logger, making NLOGX print on Novl Editor console panel
...
class gui_sink : public spdlog::sinks::base_sink<Mutex> {
  public:
    gui_sink(n_sptr<n_vector<log_item>> msg_queue){
  ...
  private:
    ...
    n_sptr<n_vector<log_item>> messages;
};

typedef n_sptr<gui_sink<n_mutex>> gui_sink_mt_t;

inline const gui_sink_mt_t gui_sink_mt(n_sptr<n_vector<log_item>> target){
