...
bool InputManager::isKeyPressed(NovlKeyCode key) {
...
bool InputManager::isMouseButtonPressed(NovlMouseButton button) {
...
void InputManager::onKeyPress(NovlKeyCode key) {
    uint16 keyIndex = static_cast<uint16>(key);
    if (keyIndex < MAX_KEYCODE_COUNT) {
...
void InputManager::onKeyRelease(NovlKeyCode key) {
    uint16 keyIndex = static_cast<uint16>(key);
    if (keyIndex < MAX_KEYCODE_COUNT) {
...
void InputManager::onMouseButtonPress(NovlMouseButton button) {
    uint8 buttonIndex = static_cast<uint8>(button);
    if (buttonIndex < MAX_MOUSEBUTTON_COUNT) {
...
void InputManager::onMouseButtonRelease(NovlMouseButton button) {
    uint8 buttonIndex = static_cast<uint8>(button);
    if (buttonIndex < MAX_MOUSEBUTTON_COUNT) {
