...
***首先尝试***把ImGui相关的操作全部放在Editor中，即：在Editor初始化的时候初始化ImGui  
...
---
```
***然后尝试***把ImGui的初始化也放在Runtime，在Runtime抽象出来一个UI类，在Editor实现一个EditorUI的子类  
---> 仍然没有解决问题，还是报错，虽然两边指针指向的地址是一致的，但是好像并不指向同一块物理内存，这是一个DLL跨边界问题 

***最终***，把ImGui render backend的初始化放在Runtime中，而ImGui context的创建放在Editor中，这样就合理了，但是结果是Render Backend需要ImGui的context，找不到context了
---> 终极解决方案就是Runtime不再作为DLL而是一个static lib存在，这样就没有DLL跨边界问题的大坑了  

> 只能说还是菜，C++学的不好，折腾半天才搞定一个很傻的问题  

所以最终，UI部分的框架是这样的：   
（非常丑陋，什么时候实在看不下去就重构，感觉写的时候已经被上面的问题折磨得没法思考）  
首先有一个UIBase Class，这个UI可以是ImGui做的Editor的UI，也可以是其他方式实现的游戏内UI  
这个UIBase是在Runtime中的
```cpp
// Novl/src/Details/UI/NovlUI.h
class UIBase{
public:
    UIBase(){}
    virtual ~UIBase(){}
...
```
---
然后在Runtime中有他的一个子类ImGuiUI，如下：  
```cpp
// Novl/src/Details/UI/ImGui/ImGuiUI.h
...
```
在这个ImGuiUI类中，进行ImGui的render backend初始化操作：  
```cpp
// ImGuiUI.cpp
void ImGuiUI::init() {
    // ...
...
```
---
然后在Editor中继承ImGuiUI，完成EditorUI： 
```cpp
// NovlEditor/src/GUI/EditorUI.h
class PanelBase;
...
```
在这个EditorUI中，才需要真正的创建一个ImGui Context，并且持有一个Panel的数组，在更新的时候对每个UI中的Panel进行更新，初始化ImGui Context如下：
```cpp
// EditorUI.cpp
EditorUI::EditorUI() {
...
```
然后在跟随Editor实例一起init/close/update即可，注意这里的update就在Runtime的tick和flush之间，**这就是为什么Runtime的tick和flush是分开的**，因为Runtime的逻辑tick完了之后，Editor也得做一下update，才能对Window进行flush，也就是调用Window的endFrame()，如下： 
```cpp
// NovlEditor.cpp
void NovlEditor::update(){
    // Runtime tick logic & render
